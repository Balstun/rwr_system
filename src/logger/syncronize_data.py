...
def sample_and_sync_h5(input_h5_path, output_h5_path, sampling_frequency, compress, resize_to, topic_types):
    ...
    with h5py.File(input_h5_path, 'r') as input_h5, h5py.File(output_h5_path, 'w') as output_h5:
        ...
        for topic, topic_type in topic_types.items():
            ...
            elif TOPIC_TO_STRING[topic_type] == "Float32":
                # Interpolate Float32 data
                scalar_data = np.array([float(topic_group[str(ts)][()]) for ts in topic_timestamps])
                interp_scalar = interp1d(
                    topic_timestamps, scalar_data, kind="linear", fill_value="extrapolate"
                )
                sensors_continuous = interp_scalar(desired_timestamps)     
                print("Created Float32 dataset")       
                output_h5.create_dataset(f"observations/{topic}", data=sensors_continuous)
